...
- Current milestone: **Reimu Layered Assets v1 — authoring contract ready, pilot assets pending** (on top of the hardened Eating Set v1 identity baseline). The consumer pipeline is production-hardened: fail-closed build/source/publish path isolation, a set-level publication transaction (all six states commit as one generation or every changed state rolls back, with explicit recovery markers if rollback itself fails), and semantically state-bound runtime manifests. The layered production system (layer contract, machine-readable layer set, builder `source_mode: layered` → Animation Plan v2, `ART ASSET REQUIRED` fail-closed behavior) is implemented and integration-tested with synthetic layers; **no real Reimu layer PNGs exist yet**. Pilot state: `task_2`. The published animation content remains the one-frame identity hold per state (see `docs/sprite-harness-integration.md`).
...
- Animation pipeline status: **production pipeline live and hardened** against Sprite Harness 0.7.0 via its public CLI/JSON contract only (no harness modules imported, no harness core changes needed); builds are deterministic (repeated builds are byte-identical), validation failures block publication, destructive build paths are isolation-checked before any deletion, and publication is a set-level transaction (all-or-nothing across the six states, recovery markers on rollback failure). The builder supports both source modes: `flattened` (plan v1, current) and `layered` (plan v2 from the layer set; blocked only on authored layer PNGs).
...
- Layered Assets v1 contract (2026-09-03): layered sources live in `assets/reimu/layered/eating/` (version-controlled, immutable, never in `build/`); the machine-readable contract is `pets/reimu/layers/eating/layer-set.json` (596×596 reference canvas, 12 layers with explicit anchors/positions/z-order, shared body construction + tier-specific expression/food per the sprite-economy rule); the human specification and QA checklist are `docs/reimu-layered-assets-v1.md`. All layer positions are provisional until measured from the authored pilot PNGs. Pilot state: `task_2` (medium complexity: held food + table food + transitional expression); switching to `task_3` requires a recorded rationale here.
...
1. Maintainer reviews the Layered Assets v1 contract: the layer decomposition, z-order, anchors, and QA checklist in `docs/reimu-layered-assets-v1.md` and `pets/reimu/layers/eating/layer-set.json`, plus the earlier identity-baseline rationale (`docs/sprite-harness-integration.md`).
2. **ART ASSET REQUIRED** — author the pilot layer PNGs for `task_2` plus the shared layers (`shared/tatami|body|head|table.png`, `task_2/eyes_open|eyes_closed|mouth|hand_right|held_food|table_food.png`; optional `hand_left`/`effects`), per the exact specification in `docs/reimu-layered-assets-v1.md`. Do not derive them from the flattened sprites and do not invoke harness M4 generation.
3. After committing pilot PNGs: measure and update the provisional positions in `layer-set.json`, set `"source_mode": "layered"` and the restrained pilot tracks (breathing/eating hand/blink/chew) on `task_2` in `animation-set.json`, run `python3 tools/build_reimu_animations.py --states task_2`, and complete the visual QA checklist (identity, motion, loop, 160 px, reduced motion).
4. Only after pilot approval: scale the same layer contract to `idle` and `task_1`–`task_5`, keeping motion intensity close while expressions follow the locked tier 0–5 emotional progression.
5. Only after layered animations exist: design the Codex standard-row performances and use the harness M5 atlas export along the boundary documented in `docs/sprite-harness-integration.md` (the full Codex atlas still lacks running/waving/jumping/failed/waiting/review/look-direction assets and is a later milestone).
6. Investigate workload integration separately; the preview's debug task count must not be presented as live Codex integration.

## 8. Current handoff status

- Blockers: real layered Reimu PNGs do not exist yet (**ART ASSET REQUIRED**, pilot `task_2`); layered animation production is gated on that art task, not on tooling.
...
- Expected result: `repository scaffold checks passed`; all pipeline tests pass (43 tests as of 2026-09-03: spec composition, path isolation, set-level transaction with failure injection, layered source composition, manifest determinism, end-to-end flattened + layered v2 integration against the real CLI).
...
- Latest completed change: hardened the consumer pipeline (build/source/publish path isolation with alias detection, set-level all-or-nothing publication transaction with recovery markers, state-bound runtime manifest loading) and established the Reimu Layered Assets v1 production system (asset contract + machine-readable layer set + builder `source_mode: layered` → Animation Plan v2, integration-tested with synthetic layers; real pilot art pending).
