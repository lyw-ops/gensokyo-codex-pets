#!/usr/bin/env python3
"""Import the approved idle-v1 exact frames as a Codex v2 idle-row source.
...
except ModuleNotFoundError:  # Direct execution: python tools/import_reimu_idle_v1.py
...
EXPECTED_ANIMATION_ID = "reimu_codex_v2_idle_v1_hair_settle_loop"
EXPECTED_PLAN_DIGEST = "sha256:b0a0df96a302a2282db7ffba9512d6f84d001298ebaaeca78611985cc3732b24"
...
EXPECTED_FRAME_SHA256 = (
    ...
    "0251947e0ca94f0ba0fa4b724d37fac242c86ba4de9591bb558c226ad4904cfe",
    "acce41a0614e4b1c15b1e779272f43e7d9204b56fc400bf95f8b60696c989523",
    "6f65a6f5a9f97039bd2c687c0c20a4c9e266f2cdfa283c5e99c4a15875bf168f",
    "acce41a0614e4b1c15b1e779272f43e7d9204b56fc400bf95f8b60696c989523",
    "0251947e0ca94f0ba0fa4b724d37fac242c86ba4de9591bb558c226ad4904cfe",
)
SOURCE_LEVELS = (0, 0, 1, 2, 1, 0)
...
def verify_harness_build(build: Path) -> list[Path]:
    ...
    if frame_plan.get("animation_id") != EXPECTED_ANIMATION_ID:
        raise ImportErrorDetail("Harness animation_id is not the approved idle-v1 loop")
    if frame_plan.get("plan_digest") != EXPECTED_PLAN_DIGEST:
        raise ImportErrorDetail("Harness plan digest is not the approved idle-v1 loop")
    ...
    if playback.get("frame_count") != 6 or playback.get("fps") != 5.0 or playback.get("loop") is not True:
        raise ImportErrorDetail("Harness idle playback contract changed")
...
def source_manifest(frame_entries: list[dict]) -> dict:
    return {
        ...
        "codex_contract": {
            ...
            "columns": 8,
            "used_columns": 6,
            "durations_ms": list(IDLE_DURATIONS_MS),
        },
        "action": "idle_relaxed",
        ...
        "motion": {
            ...
            "description": "fixed seated pose; only the image-left loose hair curl lower contour settles by 1-2px; eyes, face, food, hands, body, tea, table and tatami remain fixed",
        },
        "frames": frame_entries,
        "limitations": [
            "This is one approved Codex standard row, not a complete or installable pet atlas.",
            "The current idle pilot contains the confirmed hair-settle micro-loop only; the earlier design-layer tea-sip idea is deferred.",
...
def import_frames(build: Path, destination: Path) -> dict:
    ...
    try:
        ...
        for index, (image, level, duration_ms) in enumerate(
            zip(source_images, SOURCE_LEVELS, IDLE_DURATIONS_MS, strict=True)
        ):
            ...
            frame_entries.append(
                {
                    ...
                    "source_frame": index,
                    "hair_settle_level": level,
                }
            )

        if (staging / "frame_000.png").read_bytes() != (staging / "frame_005.png").read_bytes():
            raise ImportErrorDetail("idle row endpoints are not pixel-identical neutral frames")
        if (staging / "frame_002.png").read_bytes() != (staging / "frame_004.png").read_bytes():
            raise ImportErrorDetail("idle row intermediate settle frames are not symmetric")
...
def main() -> int:
    ...
    parser.add_argument(
        "--destination",
        type=Path,
        default=Path("pets/reimu/sprites/codex-v2/rows/idle"),
    )
