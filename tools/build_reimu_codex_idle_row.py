#!/usr/bin/env python3
"""Build and validate Reimu's approved Codex v2 idle-row source.

The tool follows the same public Sprite Harness workflow as the running-row
pilot and produces only a validated 1536x208 row strip under build/. It never
...
except ModuleNotFoundError:  # Direct execution: python tools/build_reimu_codex_idle_row.py
...
DEFAULT_SOURCE = REPO_ROOT / "pets/reimu/sprites/codex-v2/rows/idle"
DEFAULT_BUILD = REPO_ROOT / "build/codex-v2/reimu-idle-row"
EXPECTED_DURATIONS = [280, 110, 110, 140, 140, 320]
...
def validate_source(source_dir: Path) -> dict:
    ...
    if manifest.get("character") != "reimu" or contract.get("row") != "idle":
        raise BuildError("source semantic binding must be reimu/idle")
    if contract.get("sprite_version") != 2 or contract.get("row_index") != 0:
        raise BuildError("source is not bound to Codex v2 row 0")
    ...
    if contract.get("durations_ms") != EXPECTED_DURATIONS:
        raise BuildError("source idle-row durations changed")
    frames = manifest.get("frames") or []
    if len(frames) != 6:
        raise BuildError("idle row must declare six source frames")
    ...
    if actual != declared:
        raise BuildError(f"undeclared PNGs in idle source: {sorted(actual - declared)}")
    if (source_dir / "frame_000.png").read_bytes() != (source_dir / "frame_005.png").read_bytes():
        raise BuildError("idle row endpoints must be pixel-identical neutral frames")
    if (source_dir / "frame_002.png").read_bytes() != (source_dir / "frame_004.png").read_bytes():
        raise BuildError("idle row intermediate settle frames must be pixel-identical")
    return manifest
...
def plan_spec() -> dict:
    return {
        "plan_version": 1,
        "animation_id": "reimu_codex_v2_idle_hair_settle_v1",
        "canvas": {"width": 192, "height": 208, "background": "transparent"},
        "playback": {"fps": 5.454545454545454, "frame_count": 6, "loop": True},
        "anchor": {"type": "custom", "x": 0.5, "y": 1.0},
        "reduced_motion": {"mode": "hold_first_frame"},
        "tracks": [],
        "events": [
            {
                "event_id": "hair_settle_peak",
                "type": "pose_phase",
                "target": "image_left_loose_hair_curl",
                "frames": [3],
            }
        ],
        "metadata": {
            ...
            "codex_row": "idle",
            "prototype": "idle-v1-mother-locked-hair-settle",
...
def verify_row_atlas(atlas_path: Path, source_dir: Path) -> dict:
    ...
    if unused.getchannel("A").getbbox() is not None:
        raise BuildError("unused idle-row cells are not fully transparent")
    return {
...
def build_idle_row(harness: str, source_dir: Path, output_dir: Path) -> dict:
...
def main() -> int:
    ...
    try:
        ...
        qa = build_idle_row(harness, source_dir, args.build_dir.resolve())
