"""Regression tests for the approved Reimu Codex v2 review-row source."""
...
from tools.build_reimu_codex_review_row import (
    DEFAULT_SOURCE,
    BuildError,
    build_review_row,
    sha256,
...
class ReviewRowSourceTests(unittest.TestCase):
    def test_checked_in_source_is_valid(self) -> None:
        manifest = validate_source(DEFAULT_SOURCE)
        self.assertEqual(manifest["codex_contract"]["row_index"], 8)
        self.assertEqual(manifest["codex_contract"]["durations_ms"], [150, 150, 150, 150, 150, 280])
        self.assertEqual(len(manifest["frames"]), 6)

    def test_digest_tamper_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory(dir="/private/tmp") as temporary:
            copied = Path(temporary) / "review"
...
class ReviewRowHarnessIntegrationTests(unittest.TestCase):
    ...
    def test_build_export_validate_and_repeat_deterministically(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir="/private/tmp") as temporary:
            output = Path(temporary) / "review-row"
            first = build_review_row(harness, DEFAULT_SOURCE, output)
            first_atlas = (output / "row-atlas" / "atlas.png").read_bytes()
            first_qa = (output / "qa.json").read_bytes()

            second = build_review_row(harness, DEFAULT_SOURCE, output)
