"""Regression tests for the approved Reimu Codex v2 running-row pilot."""
...
class RunningRowHarnessIntegrationTests(unittest.TestCase):
    ...
    def test_build_export_validate_and_repeat_deterministically(self) -> None:
        ...
        # macOS exposes /var as a symlink to /private/var; Harness correctly
        # rejects symlink-containing export paths, so use the canonical root.
        with tempfile.TemporaryDirectory(dir="/private/tmp") as temporary:
            output = Path(temporary) / "running-row"
            first = build_running_row(harness, DEFAULT_SOURCE, output)
            first_atlas = (output / "row-atlas" / "atlas.png").read_bytes()
            first_qa = (output / "qa.json").read_bytes()

            second = build_running_row(harness, DEFAULT_SOURCE, output)
