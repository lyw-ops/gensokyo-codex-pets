#!/usr/bin/env python3
"""Build and validate Reimu's approved Codex v2 review-row source.
...
except ModuleNotFoundError:  # Direct execution: python tools/build_reimu_codex_review_row.py
...
DEFAULT_SOURCE = REPO_ROOT / "pets/reimu/sprites/codex-v2/rows/review"
DEFAULT_BUILD = REPO_ROOT / "build/codex-v2/reimu-review-row"
EXPECTED_DURATIONS = [150, 150, 150, 150, 150, 280]
...
def validate_source(source_dir: Path) -> dict:
    ...
    if manifest.get("character") != "reimu" or contract.get("row") != "review":
        raise BuildError("source semantic binding must be reimu/review")
    if contract.get("sprite_version") != 2 or contract.get("row_index") != 8:
        raise BuildError("source is not bound to Codex v2 row 8")
    ...
    if contract.get("durations_ms") != EXPECTED_DURATIONS:
        raise BuildError("source review-row durations changed")
    frames = manifest.get("frames") or []
    if len(frames) != 6:
        raise BuildError("review row must declare six source frames")
    ...
    if actual != declared:
        raise BuildError(f"undeclared PNGs in review source: {sorted(actual - declared)}")
    if (source_dir / "frame_000.png").read_bytes() != (source_dir / "frame_005.png").read_bytes():
        raise BuildError("review row endpoints must be pixel-identical neutral frames")
    if (source_dir / "frame_001.png").read_bytes() != (source_dir / "frame_004.png").read_bytes():
        raise BuildError("review row intermediate brow frames must be pixel-identical")
    if (source_dir / "frame_002.png").read_bytes() != (source_dir / "frame_003.png").read_bytes():
        raise BuildError("review row peak brow frames must be pixel-identical")
    return manifest
...
def plan_spec() -> dict:
    return {
        "plan_version": 1,
        "animation_id": "reimu_codex_v2_review_focused_brow_v1",
        "canvas": {"width": 192, "height": 208, "background": "transparent"},
        "playback": {"fps": 5.825242718446602, "frame_count": 6, "loop": True},
        "anchor": {"type": "custom", "x": 0.5, "y": 1.0},
        "reduced_motion": {"mode": "hold_first_frame"},
        "tracks": [],
        "events": [
            {
                "event_id": "focused_brow_peak",
                "type": "pose_phase",
...
def verify_row_atlas(atlas_path: Path, source_dir: Path) -> dict:
    ...
    if unused.getchannel("A").getbbox() is not None:
        raise BuildError("unused review-row cells are not fully transparent")
    return {
...
def build_review_row(harness: str, source_dir: Path, output_dir: Path) -> dict:
...
def main() -> int:
    ...
    try:
        ...
        qa = build_review_row(harness, source_dir, args.build_dir.resolve())
