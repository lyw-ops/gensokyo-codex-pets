...
**Sprite Harness pipeline stage (task_2 chew-v9 exact loop published; four Codex v2 rows approved).** The maintainer-approved **Eating Set v1** concept sheet is committed under `docs/reference/reimu/eating_set_v1/`, its six derived runtime states live under `assets/reimu/eating/`, and the [Sprite Harness](https://github.com/lyw-ops/Spirite-harness) production pipeline is in place. The confirmed 16-frame, 10 fps `chew-v9-asymmetric-cheeks` fixed-pose loop is pinned as a consumer exact-frame source, passed three identical no-publish preflights, and is now the validated `task_2` runtime animation. Its approved neutral is the static fallback; chew-v7 remains pinned as legacy and the preceding Eating Set v1 fallback is retained byte-exactly as `base-eating-set-v1-legacy.png` with provenance in `legacy-assets.json`. Separately, reviewed local Codex v2 row sources now live under `pets/reimu/sprites/codex-v2/`: `idle-v1-mother-locked-hair-settle` supplies row 0 (`idle`), `waiting-v1-mother-locked-attentive-brow` supplies row 6 (`waiting`), `chew-v7-mother-locked` still supplies row 7 (`running`), and `review-v1-mother-locked-focused-brow` supplies row 8 (`review`), each as six 192×208 cells. Their guarded builders validate exact source frames and use Harness M5 to export and independently revalidate 1536×208 row strips.

The published eating runtime now contains one animated vertical slice: `task_2` has 16 validated frames, while `idle`, `task_1`, and `task_3` through `task_5` remain deliberate one-frame identity holds. The approved Codex rows are separate atlas inputs and do not imply that Codex can select `task_2`, because the current custom-pet manifest exposes no active-task count. There is still no complete 1536×2288 atlas, installable pet package, or real workload integration; the preview's task count remains a manual debug input.
